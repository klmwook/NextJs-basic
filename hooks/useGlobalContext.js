...
//전역으로 관리 할 저장 객체 컨텍스트 생성 (Provider라는 컴포넌트 포함 : 전역 관리 할 값을 루트 컴포넌트에 전달)
export const GlobalContext = createContext();

//전역으로 관리할 값을 내부적으로 만들어서 루트 컴포넌트를 감싸서 전달 해주는 커스텀 Provider 컴포넌트 리턴 함수
export function GlobalProvider({ children }) {
	//전역으로 관리할 로그인 정보값이 담길 state와 state변경 함수 생성
